...
def region():
    ...
    if st.session_state.is_authenticated:
        ...
        if connection.is_connected():
            ...
            # Calculate OVERALL MTD revenue and footfalls for the selected date range
            ALL_summary_df = df_filtered.groupby(['Scheme','Region']).agg(
                MTD_Actual_Footfall=('Footfall', 'sum'),
                MTD_Budget_Footfall=('Budget_Footfalls', 'sum'),
                MTD_Actual_Revenue=('Revenue', 'sum'),
                MTD_Budget_Revenue=('Budget_Revenue', 'sum')
            ...
            # Rearrange the columns
            ALL_summary_df = ALL_summary_df[
                ['Region','Scheme', 'MTD_Budget_Revenue', 'MTD_Actual_Revenue', '%Arch_REV','Total_Footfall_Budget','Projected_Revenue', 'MTD_Actual_Footfall', 'MTD_Budget_Footfall', '%Arch_FF','Total_Revenue_Budget','Projected_Footfalls']
            ]
           ...
           # this context.
            ...
            fig6.update_layout(
                ...
                margin=dict(l=20, r=20, t=50, b=5)
                        ...
                        # Rearrange the columns
            ALL_summary_df = ALL_summary_df[
                ['Region','Scheme', 'MTD_Budget_Revenue', 'MTD_Actual_Revenue', '%Arch_REV',
                 'Total_Footfall_Budget','Projected_Revenue', 'MTD_Actual_Footfall',
                 'MTD_Budget_Footfall', '%Arch_FF','Total_Revenue_Budget','Projected_Footfalls']
            ]
            
            # Convert specific columns to integers
            int_columns_region_REV = ['MTD_Actual_Footfall', 'MTD_Budget_Footfall','Total_Footfall_Budget','Projected_Footfalls']
            percent_columns_region_REV = ['%Arch_FF']

            # Use .loc to set values for filtered_totals_df_footfall
            filtered_All_df.loc[:,  int_columns_region_REV] = filtered_All_df[int_columns_region_REV].astype(int).apply(
                lambda x: x.apply(lambda y: '{:,}'.format(y)))
            filtered_All_df.loc[:, percent_columns_region_REV] = filtered_All_df[percent_columns_region_REV].apply(
                lambda x: x.apply(lambda y: '{:.1%}'.format(y)))
            ...
            fig_request_by_type_ff = go.Figure(data=[go.Table(
                        header=dict(values=['Scheme','MTD_Budget','MTD_Actual','%Arch_FF','Total_Budget','Projection'],
                                    ...
                                    line_color='darkslategray',  # Border color
                                    line=dict(width=1)),  # Border width
                        cells=dict(values=[filtered_All_df["Scheme"],filtered_All_df["MTD_Budget_Footfall"],filtered_All_df["MTD_Actual_Footfall"],filtered_All_df["%Arch_FF"],filtered_All_df["Total_Footfall_Budget"],filtered_All_df["Projected_Footfalls"]],
                                fill_color=[
                                        ['rgba(0, 0, 82, 1)'],  # Blue for "Report" column
                                        ['white'] * len(filtered_All_df)  # White for "Count" column
                                    ],
                                font_color=[
                                        ['white'],  # Blue for "Report" column
                                        ['black'] * len(filtered_All_df)  # White for "Count" column
                                    ],
                                align='left',
                                font=dict(color='black', size=18),
                                line_color='darkslategray',
            ...
            # Convert specific columns to integers
            int_columns_region_REV = ['MTD_Actual_Revenue', 'MTD_Budget_Revenue','Total_Revenue_Budget','Projected_Revenue']
            percent_columns_region_REV = ['%Arch_REV']
            ...
            fig_request_by_type_Rev = go.Figure(data=[go.Table(
                header=dict(values=['Scheme','MTD_Budget','MTD_Actual','%Arch_REV','Total_Budget','Projection'],
                            ...
                            line_color='darkslategray',  # Border color
                            line=dict(width=1)),  # Border width
                cells=dict(values=[filtered_All_df["Scheme"],filtered_All_df["MTD_Budget_Revenue"],filtered_All_df["MTD_Actual_Revenue"],filtered_All_df["%Arch_REV"],filtered_All_df["Total_Revenue_Budget"],filtered_All_df["Projected_Revenue"]],
                        fill_color=[
                                ['rgba(0, 0, 82, 1)'],  # Blue for "Report" column
                                ['white'] * len(filtered_All_df)  # White for "Count" column
                            ],

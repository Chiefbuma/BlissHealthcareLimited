...
def home():
    ...
    if st.session_state.is_authenticated:
        ...
        if init_connection():
            ...
            month_number = {month: index + 1 for index, month in enumerate(calendar.month_name[1:])}
            ...
            if selected_year < current_year or (selected_year == current_year and month_number[selected_month] < current_month):
            ...
            selected_from_date = datetime(selected_year, month_number[selected_month], 1)

            selected_to_date = max_value_to_date
            ...
            df_filtered = df_Allsales[
                ...
                (df_Allsales['bill_date'].dt.month == month_number[selected_month])
            ]
           ...
           # this context.
            ...
            if 'selected_tab' not in st.session_state:
                st.session_state.selected_tab = 'MTD Revenue'
            

         
            selected_tab = ui.tabs(options=['MTD Revenue', 'MTD Footfalls'], default_value=st.session_state.selected_tab, key="main_tabs")

            # Store the selected tab in session state
            st.session_state.selected_tab = selected_tab
           
        
            if selected_tab == 'MTD Revenue':
            
            
                with card_container(key="MTDREVENUE"):
                    card_style3 = "border: 2px solid #000000; border-radius: 5px; padding: 10px; background-color:#ffffff; color:#000000; text-align: center; font-size: 15px;font-weight: bold; width: 100%; height: 200px;"
                    ui.card(
                        content=location,
                        key="MCcard2"
                    ).render()
                    ...
                    with cols[1]:
                        ui.card(title="MTD Budget", content=formatted_Rev_budget, description="+180.1% from last month", key="Revcard2").render()
                    with cols[2]:
                        ui.card(title="MTD Archievement", content=formatted_arch_rev, description="+19% from last month", key="Revcard3").render()
                    with cols[3]:
                        ui.card(title="Performance Index", content="+12,234", description="+19% from last month", key="Revcard4").render()
                    st.plotly_chart(fig_request_by_type_Rev, use_container_width=True)
                    with st.expander("DEPARTMENTAL MTD REVENUE (CASH & FFSS)"):
            ...
            if selected_tab == 'MTD Footfalls':
                st.session_state.logged_in= True
   
                
                with card_container(key="MTDFOOTFALLS"):
                    st.session_state.logged_in= True
                    ui.card (content=location,key="MCcard2").render()
                    cols = st.columns(4)
                    with cols[0]:
                        ui.card(title="MTD Footfalls", content=formatted_total_footfalls, description=f'{budget_performance_value} <br> from previous day', key="FFcard1").render()
                    with cols[1]:
                        ui.card(title="MTD Budget", content=formatted_FF_budget, description="+180.1% from last month", key="FFcard2").render()
                    with cols[2]:
                        ui.card(title="MTD Archievement", content=formatted_arch_ff, description="+19% from last month", key="FFcard3").render()
                    with cols[3]:
                        ui.card(title="Performance Index", content="+12,234", description="+19% from last month", key="FFcard4").render()

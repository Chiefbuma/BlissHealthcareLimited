    ...
    if st.session_state.is_authenticated:
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                TAT_df = pd.read_csv(uploaded_file, encoding='utf-8',
                                     usecols=columns_to_import, parse_dates=columns_to_import)
        ...
        #
            ...
            plt.title(
                'Distribution of Patient Visits by Time of Day and Effect on TAT')
            plt.xlabel('Time of Day')
            plt.xticks(range(7, 21, 3), [f"{hour} {
                       'AM' if hour < 12 else 'PM'}" for hour in range(7, 21, 3)], rotation=45)
            plt.ylabel('Patient Visits')
            # Density color bar for interpreting color
            plt.colorbar(scatter, label='Visit Density')
